...
def linear_problem(iso3):
    ...
    print('Optimizing {} costs'.format(iso3))
    for key, item in parameters.items():
        ...
        #Served Customers
        dfs = pd.DataFrame([served_customer])
        
        fileout = '{}_optimized_ev_center.csv'.format(iso3)
        fileout1 = '{}_optimized_customers.csv'.format(iso3)
        ...
        path_out_1 = os.path.join(folder_out, fileout1)
        dfs.to_csv(path_out_1, index = False)
